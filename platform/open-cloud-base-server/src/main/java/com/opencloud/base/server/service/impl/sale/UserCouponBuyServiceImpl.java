...
public class UserCouponBuyServiceImpl extends BaseServiceImpl<UserCouponMapper, UserCoupon> implements UserCouponService {
    ...
    public void saveCouponToUser(UserCouponParam userCouponParam) {
        ...
        couponQueryWrapper.lambda().eq(Coupon::getCouponTriggerId, SaleConstants.USER_COUPON_BUY)
        ...
        for (int i = 0; i < couponList.size(); i++) {
            ...
            JSONObject couponTriggerJsonjsonObject = JSON.parseObject(coupon.getCouponTriggerJson());
            List<String> productPackageIdList = (List<String>) couponTriggerJsonjsonObject.get("triggerProductPackageIdList");
                    
            if (!productPackageIdList.contains(userCouponParam.getProductPackageId().toString())) {
                continue;
            }
